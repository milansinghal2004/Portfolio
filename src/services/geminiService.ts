...
const API_KEY_STRING = typeof import.meta.env.VITE_GEMINI_API_KEY === 'string' && import.meta.env.VITE_GEMINI_API_KEY.trim() !== '' ? import.meta.env.VITE_GEMINI_API_KEY.trim() : null;

if (!API_KEY_STRING) {
  console.warn("VITE_GEMINI_API_KEY is not set or is invalid. Please set this environment variable in your .env file. Gemini features will be disabled.");
} else {
  try {
...
export const generateProjectDescriptionFromGitHub = async (
  ...
  if (!API_KEY_STRING) {
    throw new Error("Gemini API key (VITE_GEMINI_API_KEY) is not configured. Project generation disabled.");
  }
  if (!ai) {

...
const GitHubRepoForm: React.FC<GitHubRepoFormProps> = ({ onProjectGenerated }) => {
  ...
  const isApiKeyConfigured = typeof import.meta.env.VITE_GEMINI_API_KEY === 'string' && import.meta.env.VITE_GEMINI_API_KEY.trim() !== '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isApiKeyConfigured) {
        setError('Gemini API key (VITE_GEMINI_API_KEY) is not configured. Project generation is disabled. Please check your .env file.');
        return;
    }
      ...
      {successMessage && <p className="mt-3 text-sm text-green-500">{successMessage}</p>}
       {!isApiKeyConfigured && (
         <p className="mt-4 text-xs text-amber-600 dark:text-amber-400 p-3 bg-amber-50 dark:bg-amber-900/50 rounded-md">
           <strong>Note:</strong> A valid Gemini API key (VITE_GEMINI_API_KEY) was not detected or is not a valid string. Project generation will not work without it. This message is for development guidance. Ensure VITE_GEMINI_API_KEY is set in your .env file.
         </p>
       )}
    </div>
  );
